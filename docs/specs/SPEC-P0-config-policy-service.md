...
6. Given 旧 `config` 索引中存在历史 `WHITE_LIST/BLACK_LIST`，When 新服务首次读取策略，Then 自动迁移到新真源并保持可读。
7. Given 同时触发多个配置更新请求，When 并发写入发生，Then 最终状态可预测（无丢写、无重复、版本递增）。
...
- 提供一次性历史迁移（从 legacy `config` 索引读取 `WHITE_LIST/BLACK_LIST`）。
...
class PolicyConfig(BaseModel):
    ...
    version: int
    updated_at: str
    source: Literal["config_store", "legacy_config_index", "settings_defaults"]
...
### 5.4 读取优先级与迁移策略
读取顺序：
1. `ConfigStore.policy`（主路径）
2. legacy `config` 索引（仅读 `WHITE_LIST/BLACK_LIST`）
3. `settings.WHITE_LIST/BLACK_LIST`（最终兜底）

迁移规则：
- 当 `ConfigStore.policy` 为空且 legacy 有值时，执行一次迁移写入并打迁移日志。
- 迁移仅复制策略字段，不迁移 legacy 文档中的消息 offset 键。
- 迁移成功后后续只读 `ConfigStore.policy`。

### 5.5 API/Bot/下载链路接入
- API：`src/tg_search/api/routes/config.py` 改为 `Depends(get_config_policy_service)`。
  - `GET /api/v1/config` 维持 `ConfigModel`；其中策略字段来自 Service，其余字段继续来自 `settings`。
...
- 策略读取失败时必须有兜底（降级到 settings）并记录 warning 日志。
- 审计日志字段最少包含：`source`（api/bot/startup-migration）、`action`、`before_size`、`after_size`、`version`。
...
- [ ] T-P0-CPS-04 实现 legacy `config` 索引一次性迁移逻辑与日志。
- [ ] T-P0-CPS-05 在 API 依赖注入中注册 `ConfigPolicyService`，改造 `routes/config.py`。
- [ ] T-P0-CPS-06 改造 Bot 两条策略命令，移除直写 Meili `config` 索引路径。
- [ ] T-P0-CPS-07 改造 `download_and_listen` 与 `TelegramUserBot` 的策略读取入口。
- [ ] T-P0-CPS-08 补齐单元测试：幂等、去重、校验、版本冲突。
- [ ] T-P0-CPS-09 补齐集成/E2E：Bot 写 API 读、API 写下载生效、重启保持。
- [ ] T-P0-CPS-10 更新 `CLAUDE.md` 与 `tests/TESTING_GUIDELINES.md` 相关引用，保证文档一致。
...
## 8. 风险与回滚
- 风险 1：迁移逻辑误读 legacy 文档中的消息 offset 字段。  
  控制：仅白名单字段白名单复制（`WHITE_LIST`、`BLACK_LIST`）。
- 风险 2：API 与 Bot 同时写导致版本竞争。  
  控制：基于 `ConfigStore.version` 做乐观锁重试（或 409 返回）。
- 风险 3：ConfigStore 不可用时策略读取失败。  
  控制：降级到 `settings` 并明确告警日志。
- 回滚策略：保留 legacy 读兜底路径；如新 schema 失败，可临时切回 legacy 读取模式。

## 9. ADR（架构决策记录）
- ADR-CPS-001：策略配置必须单一真源（`ConfigStore.policy`），禁止并行维护多份可写状态。
- ADR-CPS-002：迁移采用“读时一次性导入”而非离线脚本，降低升级门槛。
